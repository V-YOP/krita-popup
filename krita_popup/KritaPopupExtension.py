...
class KritaPopupExtension(Extension):
    ...
    def createActions(self, window):
        ...
        for i in range(0, 10):
            ...
            toggle_action.triggered.connect(partial(self.toggle_popup, window.objectName(), i))
            
        for i in range(0, 10):
            setting_action = window.createAction(f"krita_pupup_edit_popup{i}", f"Edit Popup {i}", "tools/krita_popup_menu")
            setting_action.triggered.connect(partial(self.start_editing, window.objectName(), i))

    def start_editing(self, window_object_name: str, layout_idx: int):
        if not Krita.instance().documents():
            return
        self.__popup_provider.start_editing(get_window_from_object_name(window_object_name), layout_idx)
    ...
    def toggle_popup(self, window_object_name: str, layout_idx: int):
        ...
        if self.__popup_provider.is_popup_visible(layout_idx):
            ...
            self.__popup_provider.show_popup(get_window_from_object_name(window_object_name), layout_idx)
